...
Cada actividad de OT contiene líneas `barca.maintenance.workorder.line.material`. Se copian producto `product.product`, unidad de medida, cantidad estimada, secuencia, nota y referencia al material del aviso (`alert_line_material_id`). Los kits siguen tratándose como productos íntegros y no se explotan en componentes. La grilla principal de actividades muestra **N° materiales** y **Materiales** como resumen y ofrece el botón **Mat y Rep**, que abre un modal dedicado sin pestañas con únicamente las líneas asociadas a la actividad seleccionada. Desde ese modal se puede ejecutar **Reservar materiales de la OT**; la reserva conserva su alcance global y considera las diferencias pendientes de todas las actividades de la OT.
...
- **Notificar**: abre un modal dedicado que contiene únicamente descripción de lo realizado, resultado, fecha/hora y usuario de notificación. Exige descripción, resultado y cantidades de materiales no negativas; cambia la actividad `in_progress` a `notified` y registra fecha/hora y usuario notificador.
...
La lista no utiliza el enlace automático **Vista**. Cuando una actividad está notificada, la acción **Notificado** vuelve a abrir el mismo modal en modo lectura; desde allí Programador o Administrador pueden ejecutar **Reabrir a pendiente**.

El modal **Materiales** de la actividad de OT muestra el detalle estructurado de materiales, repuestos o kits. Mantiene visibles las cantidades estimada, solicitada, disponible en Serviteca, reservada, consumida y devuelta, además de UdM y observación; `withdrawn_quantity` continúa como dato técnico interno.
